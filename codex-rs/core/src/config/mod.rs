...
pub(crate) fn set_project_model_provider_inner(
    doc: &mut DocumentMut,
    project_path: &Path,
    ...
    // Ensure `[projects]` and `[projects."<path>"]` are explicit tables, preserving migration behavior.
    {
        let root = doc.as_table_mut();
        let existing_projects = root.get("projects").cloned();
        if existing_projects.as_ref().is_none_or(|i| !i.is_table()) {
    ...
    proj_tbl.set_implicit(false);
    proj_tbl["model_provider"] = toml_edit::value(model_provider_id.to_string());
    Ok(())
}
...
pub struct ProjectConfig {
    ...
    /// Optional model provider override for this project.
    pub model_provider: Option<String>,
}
...
impl Config {
    ...
    fn load_config_with_layer_stack(
        ...
        let model_provider_id = model_provider
            .or(active_project.model_provider.clone())
