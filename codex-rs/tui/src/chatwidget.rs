...
const DEFAULT_GENERATED_TEMPLATE_NAME: &str = "Generated Template";
const DEFAULT_GENERATED_TEMPLATE_DESCRIPTION: &str = "A prompt template generated by Codex.";
...
fn derived_template_description(name: &str) -> String {
    if name.trim().is_empty() || name == DEFAULT_GENERATED_TEMPLATE_NAME {
        DEFAULT_GENERATED_TEMPLATE_DESCRIPTION.to_string()
    } else {
...
fn coerce_template_name(name: &str) -> String {
    let normalized = collapse_whitespace(name).trim().to_string();
    if normalized.is_empty() {
        DEFAULT_GENERATED_TEMPLATE_NAME.to_string()
    } else {
        normalized
    }
}
