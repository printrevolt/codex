...
#[test]
fn template_generation_derives_name_and_description_when_missing() {
    let input = PrintRevoltTemplateDraftInput {
        name: "   ".to_string(),
        description: " \n\t ".to_string(),
        tags: Vec::new(),
        role_objective: "role".to_string(),
        procedure: "proc".to_string(),
        outputs: "out".to_string(),
        policy_defaults: "policy".to_string(),
        tooling_scope: "tools".to_string(),
    };

    let md = render_draft_template_markdown(&input);
    assert!(md.contains("name: \"Generated Template\""));
    assert!(md.contains("description: \"A prompt template generated by Codex.\""));
}
